// Edward Sillador
// * 07/6/2018 - added flip (vertical / horizontal), scale, rotation,
//               mono, bilevel
...
// error checking
// can be improved by adding __VA_ARGS__
...
// used for image resize
// generated by calc_contributions()
typedef struct contributions {
    double **weights;
    int **indices;
    int weights_sz; // weights and indices have the same size
} contributions;
...
// int putImageToFile()
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//   ppm_image_handler *handler
//    handler->filename
//    handler->imginfo.file_type
//    handler->imginfo.new_buff
...
// int getNextPixel() : get the next pixel from the image data
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//  ppm_image_handler *handler
//  handler->filesize
//  handler->index_buffer
// output:
//  pixel *pix
//  handler->index_buffer
int getNextPixel(ppm_image_handler *handler, pixel *pix)
...
// void getNextChar(): get the next character
//                    returns void
// input:
//   ppm_image_handler *handler
...
// int getNextToken() : get next token from ppm data
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//  ppm_image_handler *handler
...
// int getImageInfo() : converts the buffer to 2 dimensional image
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//  ppm_image_handler *handler
...
//void releaseBuffer() : free allocated pixel buffer
//            No return value
// input:
//   unsigned int height
// output:
...
//  int calc_contributions() : generate contributions weights / indices for scaling
//                             returns
//                               PPM_NOERROR - OK
//                               PPM_ERROR - NG
//  input:
...
int calc_contributions(int in_size, int out_size, double scale, double k_width, contributions *contrib)
{
// might slow down due to memory allocations. for performance
// improvement, this function can be simplified even better
    int x = 0;
    int y = 0;
    ...
    // indices with non zero weights / indices will be clamped out to
    // for_outweights / for_outindices
    ...
    // clam weights and indices; non zero weights will not be
    // included.
    for (y = 0; y < out_size; y++) {
        ind_w_ptr_x = 0;
        for (x = 0; x < P; x++)
...
// void calc_rot_size() : returns void
// input:
//   double angle
//   unsigned int old_width, unsigned int old_height
// output:
...
// int rotate() : perform image rotation
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//   ppm_image_handler *handler
...
//int imresize() : perform image scaling
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//   ppm_image_handler *handler
...
// int flip() : flip vertical or horizontal
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
//  ppm_image_handler *handler
...
// int image_buff_alloc() : allocate image buffer
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// input:
...
//    handler->imginfo.new_height
//    handler->imginfo.file_type
int gray(ppm_image_handler *handler)
{
    int x;
    int y;
...
//void renewBuffer() : renew buffer for next processing
//                  returns void
//ppm_image_handler *handler
// input:
//    handler->imginfo.new_buff
...
// int doProcessPPM() : process image processing
//               returns PPM_NOERROR - OK
//               returns PPM_ERROR - NG
// ppm_image_handler *handler
//  input:
...
//   handler->imginfo.width
//   handler->imginfo.height
//   handler->arg_flag.resize_enable
//   handler->arg_flag.rotate_enable
//   handler->arg_flag.gray_enable
//   handler->arg_flag.mono_enable
//   handler->arg_flag.flipv_enable
//   handler->arg_flag.fliph_enable
//  output:
//   handler->imginfo.buff
//   handler->filep
//   handler->filesize
